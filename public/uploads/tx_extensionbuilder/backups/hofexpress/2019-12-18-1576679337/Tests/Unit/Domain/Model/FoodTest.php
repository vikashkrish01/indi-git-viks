...
class FoodTest extends \TYPO3\TestingFramework\Core\Unit\UnitTestCase
{
     ...
     * @test
     */
    public function setRestaurantForObjectStorageContainingRestaurantSetsRestaurant()
    {
        $restaurant = new \Hulk\Hofexpress\Domain\Model\Restaurant();
        $objectStorageHoldingExactlyOneRestaurant = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
        $objectStorageHoldingExactlyOneRestaurant->attach($restaurant);
        $this->subject->setRestaurant($objectStorageHoldingExactlyOneRestaurant);

        self::assertAttributeEquals(
            $objectStorageHoldingExactlyOneRestaurant,
            'restaurant',
            $this->subject
        );
    }
     ...
     * @test
     */
    public function addRestaurantToObjectStorageHoldingRestaurant()
    {
        $restaurant = new \Hulk\Hofexpress\Domain\Model\Restaurant();
        $restaurantObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['attach'])
            ->disableOriginalConstructor()
            ->getMock();

        $restaurantObjectStorageMock->expects(self::once())->method('attach')->with(self::equalTo($restaurant));
        $this->inject($this->subject, 'restaurant', $restaurantObjectStorageMock);

        $this->subject->addRestaurant($restaurant);
    }

    /**
     * @test
     */
    public function removeRestaurantFromObjectStorageHoldingRestaurant()
    {
        $restaurant = new \Hulk\Hofexpress\Domain\Model\Restaurant();
        $restaurantObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['detach'])
        ...
        $restaurantObjectStorageMock->expects(self::once())->method('detach')->with(self::equalTo($restaurant));
        $this->inject($this->subject, 'restaurant', $restaurantObjectStorageMock);

        $this->subject->removeRestaurant($restaurant);
    }
}
